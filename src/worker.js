...
import * as Ably from 'ably/build/ably-webworker.min';
...
//DEVIN: Instead of pushing to a global queue, can we send the dequeued
// and converted data to Ably via the socket?
  ...
  switch (e.data.command) {
    case "init": {
      ...
      client.connection.on("connected", ()=> {
        ...
        let interval = setInterval(readFromQueue, 100);
      });
      client.connect();
      channel = client.channels.get('audio-segments');
      break;
    }
    case "send": {
        while (readFromQueue()) {
